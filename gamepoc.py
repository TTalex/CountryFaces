...
available_country_codes = [c[0] for c in cur.execute("select distinct country_code from Photos").fetchall()]
...
while True:
    ...
    available_owners = [c[0] for c in cur.execute(f"select distinct owner from Photos where country_code = '{random_country_code}'").fetchall()]
    random_owner = random.choice(available_owners)
    random_photo = cur.execute(f"select Photos.*, Country_Name_Fr, Continent_Code from Photos left join CountryCodes on country_code == Two_Letter_Country_Code where owner = '{random_owner}' and country_code = '{random_country_code}' order by random() limit 1").fetchone()

    _, _, lat, lon, direct_url, url, city, _, _, cc, country, continent_code = random_photo
